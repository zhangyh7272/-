...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF" style="border-collapse:collapse">  
 ...
 <% 
  ...
  <tr>
    ...
    <td><%=map.get("zhanghao") %></td> <td><%=map.get("xingming") %></td> <td><%=map.get("shuifei") %></td> <td><%=map.get("dianfei") %></td> <td><%=map.get("zongjine") %></td> <td><%=map.get("jiaofeiriqi") %></td> <td><%=map.get("jiaofeiren") %></td> <td align='center'><%=map.get("iszf")%>  <% if(map.get("iszf").equals("否")){%><a href="zhifu/index.jsp?id=<%=map.get("id")%>&biao=jiaofeixinxi"><font color='red'>支付</font></a> <%}%> </td>    
    <td width="138" align="center"><%=map.get("addtime") %></td>
    <td width="60" align="center"><a href="jiaofeixinxi_updt.jsp?id=<%=map.get("id")%>">修改</a>  <a href="jiaofeixinxi_list2.jsp?scid=<%=map.get("id") %>" onClick="return confirm('真的要删除？')">删除</a> <a href="jiaofeixinxi_detail.jsp?id=<%=map.get("id")%>">详细</a> </td>
  </tr>
  	<%
  }
   %>
